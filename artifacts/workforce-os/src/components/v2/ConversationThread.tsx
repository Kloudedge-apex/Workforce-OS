...
export function ConversationThread({ mode, conversation, detail, selected, onSelect }: ConversationThreadProps) {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start mb-1">
            <h4 className={cn("text-sm truncate", conversation.unread ? "font-bold text-ink-900" : "font-medium text-ink-900")}>
      ...
      <div className="flex flex-col h-full bg-paper-50 relative">
        ...
        <div className="shrink-0 p-4 border-b border-paper-200 bg-paper-100 flex items-center justify-between shadow-sm z-10">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-ink-900">AI Analysis</span>
            </div>
            <div className="h-4 w-px bg-paper-200" />
            <div className="flex items-center gap-2">
              <span className="text-xs text-ink-700">Sentiment:</span>
              <span className="text-xs font-semibold capitalize text-ink-900">{replyIntelligence.sentiment}</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className="bg-paper-200 text-ink-900 hover:bg-paper-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="border-b border-paper-200 pb-4 mb-6">
            <h2 className="text-xl font-serif text-ink-900">{conversation.subject}</h2>
            <p className="text-sm text-ink-400 mt-1">Between you and {conversation.leadName} ({conversation.leadCompany})</p>
          </div>

          {messages.map((msg) => {
            const isInbound = msg.direction === "inbound";
            return (
              <div key={msg.id} className={cn("flex max-w-[85%]", isInbound ? "ml-auto" : "")}>
                {!isInbound && (
                  <Avatar className="h-8 w-8 mr-3 mt-1 shrink-0 border border-paper-200">
                    <AvatarFallback className="bg-rust-100 text-rust-500 text-xs font-medium">YOU</AvatarFallback>
                  </Avatar>
                )}
                <div className={cn(
                  ...
                  <div className="flex items-center justify-between mb-2 gap-4">
                    <span className="text-xs font-semibold text-ink-900">{msg.senderName}</span>
