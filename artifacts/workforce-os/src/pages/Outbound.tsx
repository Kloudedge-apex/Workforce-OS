...
export default function Outbound() {
  const [activeTab, setActiveTab] = useState<ListArtifactsStatus | "ALL">(
    "PENDING_REVIEW",
  );
    ...
    <div className="flex h-full bg-paper-50">
        ...
        <div className="bg-white border-b border-paper-200 p-6 md:px-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <PolicyBadge
                policy={null}
                workspaceAuthorization={workspaceLiveAuthorization(orgSettings)}
              />
            </div>
          </div>
          ...
          >
            <div className="px-6 md:px-8 pt-4 border-b border-paper-200 bg-paper-50 shrink-0 overflow-x-auto no-scrollbar">
              <TabsList className="bg-paper-100 mb-[-1px]">
                <TabsTrigger
                  value="ALL"
                  className="data-[state=active]:bg-white data-[state=active]:text-ink-900"
                >
                  All
                </TabsTrigger>
                <TabsTrigger
                  value="PENDING_REVIEW"
                  className="data-[state=active]:bg-white data-[state=active]:text-rust-500"
                >
                  Pending
                </TabsTrigger>
                <TabsTrigger
                  value="APPROVED"
                  className="data-[state=active]:bg-white data-[state=active]:text-signal-positive"
                >
                  Approved
                </TabsTrigger>
                <TabsTrigger
                  value="SENT"
                  className="data-[state=active]:bg-white data-[state=active]:text-signal-info"
                >
                  Sent
                </TabsTrigger>
                <TabsTrigger
                  value="DELIVERY_UNKNOWN"
                  className="data-[state=active]:bg-white data-[state=active]:text-ember-500"
                >
      ...
      <div className="hidden lg:flex flex-col w-[320px] bg-paper-100 flex-shrink-0">
        <div className="p-4 border-b border-paper-200 bg-paper-50 sticky top-0 z-10 flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold text-ink-400 tracking-wider uppercase">
            Outbound activity
          </h3>
        </div>
...
function ArtifactList({ status }: { status?: ListArtifactsStatus }) {
  ...
  const {
    ...
    { status, page, limit },
    {
      query: {
        ...
        queryKey: ["listArtifacts", status, page, limit],
      },
    },
  );
  ...
  if (items.length === 0) {
    ...
    > = {
      PENDING_REVIEW: {
        ...
        title: "Queue clear",
        description:
          "No drafts are waiting on your review. New drafts land here as agents finish them.",
      },
      APPROVED: {
        icon: Check,
        title: "Nothing approved yet",
        description:
          "Approved drafts queue here before they send. Approve a pending draft to get started.",
      },
      SENT: {
        icon: Send,
        title: "No sends yet",
        description:
          "Once approved drafts go out, they'll show up here with delivery status.",
      },
      DELIVERY_UNKNOWN: {
        icon: ShieldAlert,
        title: "Nothing needs reconciliation",
        description:
          "Provider-ambiguous deliveries are quarantined here and are never retried automatically.",
      },
      FAILED: {
        icon: XCircle,
        title: "No terminal send failures",
        description:
          "Artifacts that exhaust retries without provider acceptance will appear here. They are never retried automatically.",
      },
      REJECTED: {
        icon: ThumbsDown,
        title: "No rejections",
        description:
          "Drafts you reject — and the reason why — collect here to tune future agent output.",
      },
    };
    const e = (status && empty[status]) || {
      ...
      description:
        "No outbound drafts across any status yet. Agents will populate this queue as they run.",
    };
    return (
      <EmptyState icon={e.icon} title={e.title} description={e.description} />
    );
  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="font-semibold text-ink-900 group-hover:text-rust-500 transition-colors">
                          {item.recipient.name}
                        </div>
                      ...
                      <td
                        className="px-6 py-4 text-ink-700 max-w-[300px] truncate"
                        title={item.subject}
                      >
                        {item.subject}
                      </td>
                      <td className="px-6 py-4">
                        <div
                          className={cn(
                            "inline-flex items-center gap-1.5 px-2 py-1 rounded border text-[10px] font-bold uppercase whitespace-nowrap",
                            badge.className,
                          )}
                        >
                          {item.status === "SENT" && (
          ...
          <StaggerItem key={item.id}>
            <motion.div
              ...
              onClick={(event) => {
                ...
                if (
                  target.closest("button, input, textarea, a, [role='dialog']")
                )
                  return;
