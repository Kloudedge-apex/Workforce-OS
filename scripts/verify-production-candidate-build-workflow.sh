...
# Statically enforce the protected, build-only console candidate workflow.
# Its only cloud mutation is one exact-source ACR build; Container Apps and
# every other production service remain outside its authority.
...
fail() {
  echo "ERROR: production console candidate-build workflow contract failed: $*" >&2
  exit 1
}
...
require_literal 'azure_client_id="353bce70-9a3d-4248-b910-64c596c1b9f8"'
...
require_literal 'azure_principal_object_id="0bc83fa6-91a2-4d94-9dff-f55c104cb425"'
...
require_literal 'scripts/verify-github-console-release-ci.sh "${SOURCE_SHA}"'
require_literal 'git -c core.hooksPath=/dev/null archive --format=tar "${SOURCE_SHA}"'
require_literal 'verify_pin docs/ops/production-api-upstream-url.sha256 api_upstream_url_sha256'
require_literal 'verify_pin docs/ops/production-clerk-auth.sha256 clerk_auth_sha256'
require_literal 'verify_pin docs/ops/production-clerk-publishable-key.sha256 clerk_key_sha256'
require_literal 'VITE_CLERK_PUBLISHABLE_KEY: ${{ secrets.VITE_CLERK_PUBLISHABLE_KEY }}'
require_literal 'actual_clerk_key_sha256="$(printf'
require_literal '"${actual_clerk_key_sha256}" != "${PINNED_CLERK_KEY_SHA256}"'
require_literal '"${decoded}" != '\''clerk.workforceos.xyz$'\'''
...
require_literal "--image 'workforceos-fe:{{.Run.ID}}'"
require_literal '--image "workforceos-fe:${SOURCE_SHA}"'
require_literal '--build-arg "VCS_REF=${SOURCE_SHA}"'
require_literal '--secret-build-arg "VITE_CLERK_PUBLISHABLE_KEY=${VITE_CLERK_PUBLISHABLE_KEY}"'
require_literal '--build-arg "VITE_CLERK_PUBLISHABLE_KEY_SHA256=${PINNED_CLERK_KEY_SHA256}"'
require_literal '--file Dockerfile'
require_literal '--platform linux/amd64'
require_literal '--no-logs'
require_literal 'az acr task show-run'
require_literal '"${BUILD_CONTEXT}/scripts/verify-registry-console-image.sh"'
require_literal '"${image}" "${SOURCE_SHA}" "${PINNED_CLERK_KEY_SHA256}"'
require_literal 'image="workforceosprodacr.azurecr.io/workforceos-fe@${digest}"'
require_literal 'kind: "workforce-production-candidate-artifact"'
require_literal 'role: "console"'
require_literal 'clerkPublishableKeySha256: $clerkPublishableKeySha256'
...
SECRET_EXPRESSIONS="$(grep -Eo '\$\{\{[[:space:]]*secrets\.[A-Za-z0-9_]+' \
  "${WORKFLOW}" || true)"
[[ "${SECRET_EXPRESSIONS}" == '${{ secrets.VITE_CLERK_PUBLISHABLE_KEY' ]] ||
  fail "only the exact environment-scoped Clerk publishable key may be consumed"
...
reject_pattern '(^|[[:space:]])set[[:space:]]+-[^[:space:]]*x' \
  "shell tracing could expose the Clerk build argument"
reject_pattern 'echo[^#]*VITE_CLERK_PUBLISHABLE_KEY' \
  "the Clerk publishable key must not be echoed"
reject_pattern '--build-arg[[:space:]]+"VITE_CLERK_PUBLISHABLE_KEY=' \
  "the Clerk publishable key must use ACR secret-build-arg"
...
assert_before 'Materialize private exact-commit build context' 'Verify reviewed production trust pins'
assert_before 'Verify reviewed production trust pins' 'Authenticate build-only Azure identity'
...
assert_before 'az acr build' 'verify-registry-console-image.sh'
assert_before 'verify-registry-console-image.sh' 'Upload sanitized console artifact evidence'

printf 'Production console candidate-build workflow contract verified\n'
